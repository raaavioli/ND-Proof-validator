...
iterate_proof(Prems, Goal, [[[Row, Prop, assumption]|NextInBox] | Rest], Proof) :-
        ...
        iterate_proof(Prems, Goal, Rest, Proof).
\end{minted}
som säger att om vi stöter på en rad där första elementet är en lista vars första element i sin tur har regeln ``assumption``, då måste \mintinline{iterate_proof/4} vara sant både om vi itererar igenom den inre listan såväl som resten av beviset utanför denna nestade lista. 
...
/**
        ...
        accessible from a row X. if X = e, the proposition in this 
        case is true. Any other variable would return false as Callrow.
*/
